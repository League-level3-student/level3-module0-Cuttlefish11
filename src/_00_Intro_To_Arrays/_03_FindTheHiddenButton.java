...
String boing = JOptionPane.showInputDialog("Enter a positive integer. The bigger the int, the harder the game. ");
int bwoing= Integer.parseInt(boing);
        // 4. Initialize the array of JButtons to be the size of the int
        //    created in step 3
 buttons= new JButton[bwoing];
...
JOptionPane.showMessageDialog(null, "A button will say ME. That button is the one you should click.");
        // 13. initialize the hiddenButton variable to a random number less than
        //     the int created in step 3
Random ran = new Random();
rando = ran.nextInt(bwoing+1);
hiddenButton = rando;
...
if (e.getSource() == buttons[hiddenButton]) {
	...
	JOptionPane.showMessageDialog(null, "You lost. You suck. Try again loser.");
}
